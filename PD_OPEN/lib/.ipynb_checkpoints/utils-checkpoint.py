...
from .metrics import masked_mape_np, pcc
...
def compute_val_loss(net, val_loader, criterion, sw, epoch):
    ...
    with torch.no_grad():
        ...
        for batch_index, batch_data in enumerate(val_loader):
            ...
            decoder_inputs = decoder_inputs.transpose(-1, -2)   # (B, N, T, 1)
...
def predict_and_save_results(net, data_loader, data_target_tensor, epoch, _max, _min, params_path, type):
    ...
    _max = _max.transpose(0, 1, 3, 2)
    _min = _min.transpose(0, 1, 3, 2)
    start_time = time()
    ...
    with torch.no_grad():
        ...
        data_target_tensor = data_target_tensor.transpose(0, 1, 3, 2)
...
#             pdb.set_trace()
            mae = mean_absolute_error(data_target_tensor[:, :, i].reshape(-1, 1), prediction[:, :, i].reshape(-1, 1))
            rmse = mean_squared_error(data_target_tensor[:, :, i].reshape(-1, 1), prediction[:, :, i].reshape(-1, 1)) ** 0.5
            mape = masked_mape_np(data_target_tensor[:, :, i].reshape(-1, 1), prediction[:, :, i].reshape(-1, 1), 0)
            pcc_ = pcc(data_target_tensor[:, :, i].reshape(-1, 1), prediction[:, :, i].reshape(-1, 1))
#             print('MAE: %.2f' % (mae))
#             print('RMSE: %.2f' % (rmse))
#             print('MAPE: %.2f' % (mape))
#             print('PCC: %.2f' % (pcc_))
